---
title: GitHub
description: Record the pull requests you opened and merged into your work file.
---

GitHub fetching records the pull requests you opened or merged. See
[Integrations](/integrations/) for how integration sections work in general.

## Setup

1. Fetch today's activity:

   ```sh
   swelog fetch github
   ```

   The first run asks for a
   [GitHub personal access token](https://github.com/settings/tokens) with the
   `repo` scope and stores it in your operating system keyring.

2. To fetch a previous date, provide `MM-DD-YYYY`:

   ```sh
   swelog fetch github --date 08-17-2026
   ```

   For the previous day, use `--yesterday` instead of working out the date:

   ```sh
   swelog fetch github --yesterday
   ```

## Output

The command creates or updates the `## GitHub` section before `## Log`. Its
contents are replaced by every later fetch — see
[Integration sections](/integrations/#how-they-fit-into-the-work-file).

Never store the GitHub token in `swelog.json` — see
[Authentication](/reference/authentication/).
