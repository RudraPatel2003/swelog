...
By default, it gets the issues assigned to you today, plus the ones you just moved to canceled or completed.
...
When no issues match, Swelog removes the `## Linear` section.
...
## How today differs from `--date`

`--date` asks what happened on a day, so it matches on any of the five
timestamps above and keeps whatever it finds.

The default fetch asks what is on your plate, so it keeps every unfinished issue
regardless of age, and adds a finished one only when its `completedAt` or
`canceledAt` lands on today. `updatedAt` is deliberately ignored here: an issue
you closed last week and commented on today stays out of the list.
